                    ...
                    if (currentUser != null) {
                        ...
                        db.collection("users").document(currentUser.uid).get()
                            ...
                            .addOnFailureListener {
                                _authState.value =
                                    AuthState.Error("Failed to fetch user role: ${it.message}")
                            }
                    }
                } else {
                                    ...
                                    it.sendEmailVerification()
                                        .addOnCompleteListener { emailTask ->
                                            if (emailTask.isSuccessful) {
                                                _authState.value =
                                                    AuthState.Success("Account created! Please check your email for verification")
                                            } else {
                                                _authState.value =
                                                    AuthState.Error("Account created but failed to send verification email: ${emailTask.exception?.message}")
                                            }
                                        }
                                } else {
                                    _authState.value =
                                        AuthState.Error("Failed to update profile: ${profileTask.exception?.message}")
                                }
                            }
                    }
                } else {
    ...
    fun resendVerificationEmail() {
        ...
        currentUser.sendEmailVerification()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    _authState.value =
                        AuthState.Success("Verification email sent. Please check your inbox")
                } else {
                    when (task.exception?.message) {
                        "A network error (such as timeout, interrupted connection or unreachable host) has occurred." ->
                            _authState.value =
